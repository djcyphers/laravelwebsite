...
$(document).ready(function() {
    ...
    function handleTouchMove(evt) {
        ...
        if ( Math.abs( xDiff ) > Math.abs( yDiff ) ) {/*most significant*/
            ...
            /* left swipe */
            /* check where we are */
                if ((!$('a.link').hasClass('selected')) || ($("a.link[href*='#box1']").hasClass('selected'))){
                    setPosition('#box2');
...
$(document).ready(function() {
    ...
    $( ".album-container" ).on('click', function() {
        ...
        $('<div/>', {class:'container fb-container container-fluid row col-lg-12'}).appendTo('#gallery');

        // Create close button for album images section
        var bsButton = '<div class="outerclose" aria-label="Close"><div class="innerclose"><label>Back</label></div></div>';
        ...
        $('.album-image').each(function() {
            ...
            if(albumList === albumId) {
                $('<a/>', {
                    href: path,
                    class: 'fancybox image-wrapper card text-white border-0 bg-transparent mb-3 col-md-6 col-lg-4 collapse',
        ...
        $('.fancybox').each(function() {
            ...
            $(this).append("<img class='image img-fluid rounded mx-auto d-block' src='" + imageUrl + "' width='100%'/>");
        });
...
(function($) {
    ...
    // svg fallback
    if (!Modernizr.svg) {
        $(".header__logo img").attr("src", "{{ asset('images/logo.png'}}");
    }
